...
# Listing/aggregator handles whose social profiles belong to the data source,
# not the project. These leak into project pages via site header/footer chrome
# (e.g. Coinranking's own LinkedIn appears on every coin page), so they must be
# stripped from any social extracted on an aggregator page. No real crypto
# project's handle contains these exact tokens, so the filter is safe globally.
...
TELEGRAM_BLOCKLIST = AGGREGATOR_BLOCKLIST + (
    "telegram",
    "share",
    "joinchat/aaaa",
)
...
def extract_linkedin(html):
    ...
    urls = []
    for item in raw:
        if any(bad in item.lower() for bad in AGGREGATOR_BLOCKLIST):
            continue
